  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      const response = await fetch(
        buildApiUrl("/api/outline_analysis_stream"),
        {
          method: "POST",
    ...
    <div className="flex h-full flex-col bg-muted/10">
      <div className="flex flex-col gap-4 p-5 border-b border-border/40 shrink-0">
        ...
        <div className="flex flex-col gap-2">
          <h2
            className="font-semibold text-lg truncate tracking-tight text-foreground"
          ...
          <div className="flex items-center gap-2 mt-1">
            <Button
              ...
              variant="secondary"
              size="sm"
              className="h-8 flex-1 justify-center rounded-lg text-xs bg-background hover:bg-muted shadow-sm border border-border/40"
            >
          ...
          <div>
            ...
            <div className="space-y-1">
              {QUICK_ACTIONS.map((action) => (
                ...
                >
                  <div className="line-clamp-2 leading-relaxed group-hover:line-clamp-none transition-all">
                    {action}
                  </div>
                </button>
              ))}
    ...
    <TooltipProvider delayDuration={300}>
      <div className="flex h-screen w-full bg-background text-foreground overflow-hidden font-sans">
          ...
          <div className="p-3 sm:p-5 bg-background/95 backdrop-blur border-t border-border/40 shrink-0 relative z-20">
            <div className="mx-auto w-full max-w-3xl relative">
              ...
              <div className="mt-3 text-center text-[11px] text-muted-foreground/50 flex items-center justify-center gap-1.5">
                <Sparkles size={11} className="text-muted-foreground/40" /> AI
                生成内容仅供参考，请结合实际创作调整
              </div>
            </div>
          </div>
        ...
        <Sheet open={mobileHistoryOpen} onOpenChange={setMobileHistoryOpen}>
          <SheetContent
            side="left"
            className="w-[85vw] max-w-sm p-0 flex flex-col border-r-border/40"
          >
            <div className="p-4 border-b border-border/40 shrink-0">
              <SheetTitle className="text-lg">历史侧栏</SheetTitle>
              <SheetDescription className="mt-1 text-xs">
                查看过往提问，并一键跳回对应位置继续分析。
              </SheetDescription>
            </div>
