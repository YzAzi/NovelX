...
function matchCharacter(node: CharacterGraphNode, query: string) {
  ...
  const haystack = [node.name, node.description ?? "", ...(node.aliases ?? [])]
    .join(" ")
    .toLowerCase();
...
export function CharacterGraph() {
  const { currentProject, graphUpdateVersion, loadProject, setError } =
    useProjectStore();
  ...
  const chapterLabel = useMemo(() => {
    ...
    return chapter
      ? `${chapter.narrative_order}. ${chapter.title}`
      : "当前章节";
  ...
  const relationFilteredGraph = useMemo(() => {
    ...
    return {
      nodes: chapterFilteredGraph.nodes.filter((node) =>
  ...
  const visibleCharacters = useMemo(() => {
    ...
    return [...relationFilteredGraph.nodes]
      ...
      .sort((left, right) => {
        const diff =
          (right.appearances?.length ?? 0) - (left.appearances?.length ?? 0);
  ...
  const displayGraph = useMemo(() => {
    ...
    return {
      nodes: relationFilteredGraph.nodes.filter((node) =>
        contextualIds.has(node.id),
      ),
  ...
  const relationSummaries = useMemo(() => {
    const items: RelationSummary[] = displayGraph.links.map((link) => {
      ...
      return {
        ...
        focus:
          !!focusCharacterId &&
          (sourceId === focusCharacterId || targetId === focusCharacterId),
      };
    });
  ...
  useEffect(() => {
    ...
    setPalette({
      ...
      surfaceStrong:
        read("--cg-surface-strong") || defaultPalette.surfaceStrong,
  ...
  useEffect(() => {
    ...
    const target = displayGraph.nodes.find(
      (node) => node.id === selectedCharacterId,
    );
  ...
  const handleOpenRelationEditor = useCallback(
  ...
  const handleOpenRelationEditorFromLink = useCallback(
    (link: CharacterGraphLink) => {
      const sourceId = getNodeId(link.source);
      const targetId = getNodeId(link.target);
    ...
    <div className="rounded-xl border border-[var(--cg-border)] bg-[var(--cg-surface)] p-4 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h3 className="mt-1 font-serif text-xl text-[var(--cg-ink)]">
            筛选与导航
          </h3>
        </div>
      ...
      <div className="mt-4 space-y-4">
        <div className="space-y-2">
          <label className="text-xs font-medium text-[var(--cg-muted)]">
            按章节聚焦
          </label>
          <select
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-semibold text-[var(--cg-ink)]">
                        {node.name}
                      </p>
            ...
            <div className="mt-4 grid grid-cols-3 gap-2">
              {[
                ...
                >
                  ...
                  <p className="mt-1 text-lg font-semibold text-[var(--cg-ink)]">
                    {item.value}
                  </p>
                </div>
              ))}
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-sm font-semibold text-[var(--cg-ink)]">
                        {item.label}
                      </p>
                      {item.focus ? (
  ...
  return (
    ...
    >
          ...
          <div className="rounded-2xl border border-[var(--cg-border)] bg-[var(--cg-surface)] p-4 shadow-sm">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <h3 className="mt-1 font-serif text-2xl text-[var(--cg-ink)]">
                  关系网络
                </h3>
                <p className="mt-1 text-sm text-[var(--cg-muted)]">
            ...
            >
              ...
              <div className="absolute left-4 top-4 z-10 flex flex-wrap gap-2">
                {[
                  {
                    label: "当前焦点",
                    tone: "bg-[var(--cg-accent)] text-[var(--cg-bg)]",
                  },
                  {
                    label: "关联角色",
                    tone: "bg-[var(--cg-accent-soft)] text-[var(--cg-accent)]",
                  },
                  {
                    label: "普通角色",
                    tone: "bg-white text-[var(--cg-muted)] border border-[var(--cg-border)]",
                  },
                ].map((item) => (
                  <span
                    key={item.label}
                    className={cn(
                      "rounded-full px-3 py-1 text-[11px] shadow-sm",
                      item.tone,
                    )}
                  >
                    {item.label}
                ...
                <div
                  ...
                  style={{
                    left: Math.min(
                      ...
                      Math.max(graphSize.width - 280, 16),
                    ),
                    top: Math.min(
                      pointerPosition.y + 16,
                      Math.max(graphSize.height - 140, 16),
                    ),
                  }}
                >
              ...
              {loading ? (
                ...
                <div className="flex h-full min-h-[420px] flex-col items-center justify-center gap-2 px-6 text-center text-sm text-[var(--cg-muted)] sm:min-h-[520px] xl:min-h-[620px]">
                  <p className="text-base text-[var(--cg-ink)]">
                    当前没有可展示的角色关系
                  </p>
                ...
                <ForceGraph2D
                  ...
                  linkColor={(link) => {
                    ...
                    if (
                      hoveredRelationId &&
                      getRelationId(graphLink) === hoveredRelationId
                    ) {
                      return palette.accent;
                    }
                    if (
                      focusCharacterId &&
                      (source === focusCharacterId ||
                        target === focusCharacterId)
                    ) {
                      return palette.accent;
                    }
                    return palette.link;
                  ...
                  onLinkClick={(link) =>
                    handleOpenRelationEditorFromLink(link as CharacterGraphLink)
                  }
                  onLinkHover={(link) =>
                    setHoveredRelationId(
                      link ? getRelationId(link as CharacterGraphLink) : null,
                    )
                  }
                  onNodeHover={(node) =>
                    setHoveredCharacterId(
                      (node as CharacterGraphNode | null)?.id ?? null,
                    )
                  }
                  nodeCanvasObject={(node, ctx, globalScale) => {
                    ...
                    const isNeighbor =
                      ...
                      relationSummaries.some(
                        (item) =>
                          item.focus &&
                          (item.sourceId === graphNode.id ||
                            item.targetId === graphNode.id),
                      );
                    const degree = degreeByNodeId.get(graphNode.id) ?? 0;
                    const appearanceCount = graphNode.appearances?.length ?? 0;
                    const radius =
                      (11 +
                        Math.min(appearanceCount, 6) * 1.7 +
                        Math.min(degree, 5) * 1.4) /
                      globalScale;
                    ...
                    const badgeWidth =
                      ctx.measureText(badge).width + 10 / globalScale;
                    const boxWidth = Math.max(
                      radius * 2.4,
                      textWidth + 22 / globalScale,
                    );
                  ...
                  linkCanvasObject={(link, ctx, globalScale) => {
                    ...
                    const focused =
                      !!focusCharacterId &&
                      (sourceId === focusCharacterId ||
                        targetId === focusCharacterId);
                    const hovered =
                      getRelationId(graphLink) === hoveredRelationId;
                    const showLabel =
                      hovered || focused || displayGraph.links.length <= 10;
                    if (!showLabel) {
                      return;
                    }
                    ...
                    const fontSize =
                      (hovered ? 10.5 : focused ? 10 : 8.5) / globalScale;
                    ...
                    ctx.strokeStyle =
                      hovered || focused ? palette.accent : palette.borderSoft;
                    ctx.lineWidth = 1 / globalScale;
                    ctx.beginPath();
                    ctx.roundRect?.(
                      x - width / 2,
                    ...
                    ctx.fillStyle =
                      hovered || focused ? palette.accent : palette.muted;
                    ctx.textAlign = "center";
                    ctx.textBaseline = "middle";
      ...
      <Sheet open={mobilePanelOpen} onOpenChange={setMobilePanelOpen}>
        <SheetContent
          side="bottom"
          className="h-[82vh] rounded-t-[14px] border-t border-[var(--cg-border)] bg-[var(--cg-surface)] p-0 xl:hidden"
          style={cssVars}
        >
          ...
          <Tabs
            ...
            onValueChange={(value) =>
              setMobilePanelTab(value as "filters" | "roster" | "relations")
            }
            ...
            <div className="grid flex-1 gap-4 overflow-hidden px-4 pb-4 sm:gap-6 sm:px-6 sm:pb-6 lg:grid-cols-[260px_minmax(0,1fr)]">
              <aside className="space-y-4">
                <div className="rounded-xl border border-[var(--cg-border)] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(244,220,201,0.4))] p-4">
                  ...
                  <p className="mt-3 font-serif text-3xl text-[var(--cg-ink)]">
                    {selectedCharacter.name}
                  </p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    {
                      label: "章节筛选",
                      value: selectedChapterId === "all" ? "全部" : "已选",
                    },
                  ].map((item) => (
                    <div
                      key={item.label}
                    ...
                    >
                      ...
                      <p className="mt-1 text-xl font-semibold text-[var(--cg-ink)]">
                        {item.value}
                      </p>
                    </div>
                  ))}
      ...
      <Dialog open={showAddCharacter} onOpenChange={setShowAddCharacter}>
        ...
        >
          <DialogHeader>
            <DialogTitle className="font-serif text-2xl text-[var(--cg-ink)]">
              新增角色
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
        ...
        >
          ...
          <DialogFooter className="mt-4 sm:justify-between">
            ...
            <div className="flex gap-3">
              <Button
                variant="secondary"
                onClick={() => setShowEditRelation(false)}
                className="rounded-full"
              >
                取消
              </Button>
