  ...
  return (
    ...
    >
      <Handle
        ...
        isConnectable={false}
        className="!bg-muted-foreground/40 !h-3.5 !w-1 !rounded-full !border-none"
      />
...
export function StoryVisualizer() {
  ...
  const updateActiveSceneFromViewport = useCallback(() => {
    ...
    const centerX =
      -viewport.x / viewport.zoom + rect.width / 2 / viewport.zoom;
    const centerY =
      -viewport.y / viewport.zoom + rect.height / 2 / viewport.zoom;
  ...
  const handleNodeDragStop = useCallback(async () => {
    ...
    const sortedFlowNodes = [...currentFlowNodes].sort(
    ...
    const hasChanged = sortedIds.some(
      (id, index) => id !== currentOrderIds[index],
    );
    if (!hasChanged) {
      return;
    }
  ...
  const activeSceneLabel =
    orderedStoryNodes
      .find((node) => node.id === activeSceneId)
      ?.title?.trim() || "未选择场景";
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-1.5">
                    ...
                    <span
                      className={cn(
                        "line-clamp-1 text-[12px] font-medium leading-tight",
  ...
  return (
    <div
      ref={wrapperRef}
      className="flex h-full flex-col overflow-hidden bg-background lg:flex-row"
    >
          ...
          <>
            ...
            >
              <Background
                ...
                size={1}
                color="currentColor"
                className="text-border/35 subtle-grid"
              />
              <Controls
                position="bottom-right"
                className="!bg-card/80 !backdrop-blur !border !border-border/60 !rounded-xl !shadow-md [&>button]:!border-border/40 [&>button:hover]:!bg-primary/10 [&>button]:!text-foreground"
              />
              <MiniMap
                pannable
                zoomable
            ...
            <div className="absolute inset-x-3 bottom-3 flex items-center justify-between gap-2 rounded-2xl border border-border/60 bg-card/88 px-3 py-2 text-xs text-muted-foreground shadow-sm backdrop-blur-sm lg:hidden">
              <div className="min-w-0">
                ...
                <div>{Math.round(zoomLevel * 100)}% 缩放</div>
              </div>
              <Button
                variant="ghost"
                size="sm"
      ...
      <Sheet open={mobileSceneSheetOpen} onOpenChange={setMobileSceneSheetOpen}>
        <SheetContent
          side="left"
          className="w-[88vw] max-w-none p-0 sm:max-w-sm lg:hidden"
        >
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent className="max-h-[88vh] w-[95vw] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              粘贴文本或上传 .txt/.md 文件。注意：导入将<b>覆盖</b>
              当前项目的所有节点。
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
